...
type Props = {
  ...
  setProcessType: (type: ProcessType) => void | Promise<void>;
...
export function CompanyProcessTab({
  processType,
  setProcessType,
  loadingProcess,
  run,
...
>
  ...
  >
    {(["ENTRADA", "SAIDA"] as ProcessType[]).map((t) => {
            ...
            <div>
              ...
              <div style={{ fontSize: 13, color: UI.textSoft, marginTop: 5 }}>
                {run
                  ...
                  : "O processo começa no primeiro check"}
              </div>
            </div>
      ...
      {loadingProcess ? (
        <Loading message="Carregando processo..." />
      ) : processTemplate?.sections?.length === 0 ? (
        <Empty message="Nenhum template/processo encontrado." />
      ) : (
        <div style={{ display: "grid", gap: 16 }}>
          {processTemplate?.sections?.map((section: any) => (
            <div
              key={section.id}
              style={{
                ...cardShellStyle(),
