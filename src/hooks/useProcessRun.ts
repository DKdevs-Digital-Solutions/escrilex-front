...
import {
  ItemStatus,
  getProcessRun,
  updateProcessItemObservation,
  updateProcessItemStatus
} from "../repository/processRun.repository";

export function useProcessRun(runId: string) {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const data = await getProcessRun(runId);
      setRun(data);
    } catch (e: any) {
      toast(e?.response?.data?.message || e.message || "Erro ao carregar process", "error");
    } finally {
      setLoading(false);
    }
  }, [runId, toast]);
  ...
  const setStatus = useCallback(
    async (itemRunId: string, status: ItemStatus) => {
      setSaving(itemRunId);
      try {
        await updateProcessItemStatus(itemRunId, status);
        await load();
      } catch (e: any) {
  ...
  const setObservation = useCallback(
    async (itemRunId: string, observation: string) => {
      setSaving(itemRunId);
      try {
        await updateProcessItemObservation(itemRunId, observation);
      } catch (e: any) {
